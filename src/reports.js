...
export function createReports({ state, orders, api, render, persist, notice, refresh, printReceipt, connectCashier, printText, loadOpenBill, money, esc }) {
  ...
  const summary = (c) => `${totals(c.total_penjualan,c.total_expenses)}<h6>Ringkasan Transaksi Lunas</h6>${table(["Cash","QRIS","Kartu Kredit","Cafe","Carwash","Detailing"],`<tr>${[c.cash,c.qris,c.card,c.cafe,c.carwash,c.detailing].map(v=>`<td>${money(v || 0)}</td>`).join("")}</tr>`,"")}<h6>Pengeluaran</h6>${expenseTable(c.expenses || [])}`;
  ...
  function closingView() {
    ...
    return wrap("Closing Harian",`<p class="text-muted">Satu rekap terbaru untuk setiap tanggal closing.</p>${cached()}${filter}${totals(rows.reduce((s,r)=>s+Number(r.total_penjualan),0),rows.reduce((s,r)=>s+Number(r.total_expenses),0))}${table(["Tanggal","Total Penjualan","Cash","QRIS","Kartu","Cafe","Carwash","Detailing","Pengeluaran","Saldo Bersih","Detail"],rows.map(r=>`<tr><td>${esc(r.tanggal.split("-").reverse().join("-"))}${r.local?'<small class="d-block text-warning">Menunggu sinkronisasi</small>':""}</td>${[r.total_penjualan,r.cash,r.qris,r.card,r.cafe,r.carwash,r.detailing || 0].map(v=>`<td>${money(v)}</td>`).join("")}<td><span class="badge badge-danger">${money(r.total_expenses)}</span></td><td><strong>${money(r.net)}</strong></td><td>${r.expenses?.length?btn("closing-detail","Lihat","info",String(r.id)):"-"}</td></tr>`).join(""),"Belum ada data closing untuk filter ini.")}`);
  }
  function modal() {
    if (!ui.modal) return "";
