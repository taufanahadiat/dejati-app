...
if ($path === 'catalog' && $_SERVER['REQUEST_METHOD'] === 'GET') {
  ...
  $categories = [['id' => 'all', 'label' => 'All', 'icon' => 'grid_view'], ['id' => 'carwash', 'label' => 'Carwash', 'icon' => 'local_car_wash'], ['id' => 'detailing', 'label' => 'Detailing', 'icon' => 'auto_awesome']];
  ...
  $q = mysqli_query($conn, 'SELECT id_produk id, produk name, biaya price, variant, nama_var, biaya_var, foto FROM tb_datacarwash ORDER BY produk'); while ($row = mysqli_fetch_assoc($q)) $items[] = ['id' => (string)$row['id'], 'name' => $row['name'], 'price' => (int)$row['price'], 'variant' => (int)($row['variant'] ?? 0), 'nama_var' => $row['nama_var'] ?? '', 'biaya_var' => $row['biaya_var'] ?? '', 'category' => 'carwash', 'categoryLabel' => 'Carwash', 'categoryIcon' => 'local_car_wash', 'cartType' => 'carwash', 'image' => 'https://pos.dejaticoffee.com' . cafe_product_thumb_url($row['foto'] ?? '')];
  $q = mysqli_query($conn, 'SELECT id_produk id, produk name, biaya price, variant, nama_var, biaya_var, foto FROM tb_datadetailing ORDER BY produk'); while ($row = mysqli_fetch_assoc($q)) $items[] = ['id' => (string)$row['id'], 'name' => $row['name'], 'price' => (int)$row['price'], 'variant' => (int)($row['variant'] ?? 0), 'nama_var' => $row['nama_var'] ?? '', 'biaya_var' => $row['biaya_var'] ?? '', 'category' => 'detailing', 'categoryLabel' => 'Detailing', 'categoryIcon' => 'auto_awesome', 'cartType' => 'detailing', 'image' => 'https://pos.dejaticoffee.com' . cafe_product_thumb_url($row['foto'] ?? '')];
