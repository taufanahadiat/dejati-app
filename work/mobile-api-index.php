...
if ($path === 'orders' && $_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  mysqli_begin_transaction($conn); try {
    ...
    $createdAt = $timestamp !== false && $timestamp <= time() + 300 ? (new DateTimeImmutable('@'.$timestamp))->setTimezone(new DateTimeZone('Asia/Jakarta'))->format('Y-m-d H:i:s') : date('Y-m-d H:i:s');
